    ...
    try {
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        if (session.metadata && session.metadata.kind === 'rect') {
          let meta = null;
          if (session.metadata.meta) {
            try { meta = JSON.parse(session.metadata.meta); } catch {}
          }
          const payload = {
...
app.use(
  cors({
    origin: [
      ...
      'http://localhost:5500',
      `http://localhost:${PORT}`,
      process.env.PUBLIC_BASE_URL || 'https://pixel-world-cup.onrender.com'
    ],
  })
);
...
app.post('/api/purchase-rect/checkout', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // 👉 On passe tout le meta vers le webhook sous forme JSON string
      metadata: {
        kind:'rect',
